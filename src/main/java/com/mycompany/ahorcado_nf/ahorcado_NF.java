...
public class ahorcado_NF {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            System.out.println("Introducir nombre Jugador: ");
            jugador = sc.nextLine();
            jugada = 0;//Reseteamos la jugada a 0 para evitar problemas en los diferentes jugadores
            ...
            if (controlJuego(palabra)) {
                ...
                System.out.println("Has perdido " + jugador);
            }
            
            if (jugada <= record) {
                jugRecord = jugador;
                record = jugada;
                System.out.print("NUEVO ");//Añadimos nuevo si se ha batido el record
            }
            //Enseñamos el record
            System.out.println("RECORD " + record + " JUGADOR " + jugRecord);
            //Bucle para aceptar solo si o no
            do {
                System.out.println("¿Nueva Partida? (si/no)");
                respuesta = sc.nextLine().toLowerCase();
            } while (!respuesta.equals("si") && !respuesta.equals("no"));
            continuar = respuesta.equals("si") ? true : false;
        } while (continuar);
